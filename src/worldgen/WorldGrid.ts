import { IGlobeOptions, EMapMode, WorldData, ICellGroupOptions, ICellGroupData, ICellGroupTooltipData, CellWorldData, WorldGridData } from '../types';
...
export class WorldGrid {
  ...
  constructor(public world: World) {
    ...
    this.cellGroupIndices = new Int32Array(new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT * this.world.mesh.numRegions));
    this.cellGroupUpdates$ = new Subject<ICellGroupData>();
    this.cellGroupID = 0;

    this.cellPopulationCount = new Int32Array(new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT * this.world.mesh.numRegions));
    ...
    for (let r = 0; r < this.world.mesh.numRegions; r++) {
      this.graph.addNode(r, {
        r,
        elevation: this.world.r_elevation[r],
      });
      const neighbors = this.world.mesh.r_circulate_r([], r);
      for (const n of neighbors) {
        this.graph.addLink(r, n, {
          deltaHeight: this.world.r_elevation[n] - this.world.r_elevation[r],
        });
      }
    }
  ...
  getCellData(cell: number): CellWorldData {
    return {
      globe: this.world.getCellData(cell),
      cellGroup: this.cellCellGroup.get(cell),
    }
  }

  distanceBetweenCells(cellA: number, cellB: number) {
    const [latA, longA] = this.world.getLatLongForCell(cellA);
    const [latB, longB] = this.world.getLatLongForCell(cellB);
  ...
  calculateCellGroup(cellGroup: CellGroup) {
    ...
    for (const cell of cellGroup.cells) {
      const xyz = this.world.coordinatesForCell(cell);
    ...
    for (const cell of cellGroup.cells) {
      let cellSides = [];
      this.world.mesh.r_circulate_s(cellSides, cell);
      for (const s of cellSides) {
        sides.push(s);
        const begin_r = this.world.mesh.s_begin_r(s);
        const end_r = this.world.mesh.s_end_r(s);
        const inner_t = this.world.mesh.s_inner_t(s);
        const outer_t = this.world.mesh.s_outer_t(s);
        const p1 = this.world.t_xyz.slice(3 * inner_t, 3 * inner_t + 3);
        const p2 = this.world.t_xyz.slice(3 * outer_t, 3 * outer_t + 3);
